...
export class MessagesPage implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const pendingContactRole = (snapshot['contactRole'] ?? null) as 'volunteer' | 'ngo' | null;
  ...
  loadConversations(
    ...
    pendingContactRole: 'volunteer' | 'ngo' | null = null
  ): void {
  ...
  openConversationWithUser(
    ...
    contactRole: 'volunteer' | 'ngo' | null = null
  ): void {
    ...
    this.messageService.getMessageHistory(userId)
      ...
      .subscribe({
        next: (res) => {
          ...
          //  1. Passed-in contactName/contactRole (from "Contact NGO" query params — most reliable)
          ...
          const resolvedRole: 'volunteer' | 'ngo' = contactRole
            ?? searchResult?.role
            ?? 'volunteer';
