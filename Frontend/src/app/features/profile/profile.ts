...
export class Profile implements OnInit {
  ...
  addSkill(skillName?: string): void {
    const name = (skillName ?? this.skillInput).trim();
    if (!name) return;
    const current = this.skillsList;
    if (!current.some((s: string) => s.toLowerCase() === name.toLowerCase())) {
      const updated = [...current, name];
      this.profileForm.patchValue({ skills: updated.join(', ') });
  ...
  removeSkill(index: number): void {
    ...
    this.profileForm.patchValue({ skills: current.join(', ') });
    this.profileForm.markAsDirty();
  }
    ...
    this.profileService.getProfile().subscribe({
      next: (response) => {
        ...
        this.profileForm.patchValue({
          ...
          secondaryCity:  user.locations?.secondary?.[0]?.city  ?? '',
          secondaryState: user.locations?.secondary?.[0]?.state ?? '',
  ...
  saveProfile(): void {
    ...
    const secondaryCity  = (formValue.secondaryCity  ?? '').trim();
    const secondaryState = (formValue.secondaryState ?? '').trim();
    ...
    if (this.currentRole === 'ngo') {
      wasteTypes = Array.from(this.selectedWasteTypes);
    }
    ...
    const payload: Record<string, unknown> = {
      ...
      skills,
      // Always send locations so the backend can set locations.primary/secondary
      locations: {
        primary: {
          city:  primaryCity  || undefined,
          state: primaryState || undefined,
        },
        secondary: (secondaryCity || secondaryState)
          ? [{ city: secondaryCity || undefined, state: secondaryState || undefined }]
          : [],
      },
    };
  ...
  sendOtp(): void {
    ...
    this.authService.sendChangePasswordOtp().subscribe({
      next: (response) => {
        ...
        this.otpSent = true;
        this.otpSuccessMessage = response.message || 'OTP sent successfully to your registered email.';
      ...
      error: (error) => {
        this.sendingOtp = false;
        this.otpErrorMessage = error.error?.message || 'Failed to send OTP. Please try again.';
