...
export class SocketService {
  ...
  private accountSuspendedSubject  = new Subject<{ message?: string }>();
  ...
  /** Emits when the backend suspends this user's account mid-session. */
  readonly onAccountSuspended$ = this.accountSuspendedSubject.asObservable();
  ...
  connect(token: string): void {
    ...
    // ── account:suspended — emitted when an admin suspends this user ────
    // Backend docs (admin-api.md): fires to user:<userId> room, then
    // force-disconnects the socket. We propagate via onAccountSuspended$
    // so Layout can clear auth state and navigate to /login.
    this.socket.on('account:suspended', (data: { message?: string }) => {
      this.zone.run(() => this.accountSuspendedSubject.next(data));
    });
