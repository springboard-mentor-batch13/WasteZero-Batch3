...
export class OpportunityDetail implements OnInit, OnDestroy {
  ...
  // M3: Contact NGO button — visible to volunteers only when a real NGO (not admin) is the creator
  readonly canContactNgo = computed(() => {
    if (!this.isVolunteer() || !this.ngoId()) return false;
    // If ngo_id is populated as an object, verify the creator role is 'ngo'
    const ngo = this.opportunity()?.ngo_id;
    if (typeof ngo === 'object' && ngo !== null && 'role' in ngo) {
      return (ngo as any).role === 'ngo';
    }
    // ngo_id is a plain string ID — we can't verify role without extra API call.
    // Default: show the button (existing conversations will work normally).
    return true;
  });
  ...
  // M3: Navigate to Messages with NGO pre-selected (pass name+role so Messages
  // page can build a correct synthetic conversation without a second API call)
  contactNgo(): void {
    const id = this.ngoId();
    if (!id) return;
