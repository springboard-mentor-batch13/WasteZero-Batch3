...
export class ApplicationReview implements OnInit, OnDestroy {
  ...
  readonly isNgo   = computed(() => this.authService.currentUser()?.role === 'ngo');
  ...
  // M3: Extract volunteer username from populated volunteer_id
  getVolunteerUsername(app: Application): string | null {
    if (typeof app.volunteer_id === 'object' && app.volunteer_id !== null) {
      return (app.volunteer_id as any).username ?? null;
    }
    return null;
  }

  // M3: Get volunteer ObjectId for messaging
  getVolunteerId(app: Application): string | null {
    if (typeof app.volunteer_id === 'object' && app.volunteer_id !== null) {
      return (app.volunteer_id as any)._id ?? null;
    }
    if (typeof app.volunteer_id === 'string') return app.volunteer_id;
    return null;
  }

  // M3: Navigate to Messages with volunteer pre-selected (NGO only)
  contactVolunteer(app: Application): void {
    const id = this.getVolunteerId(app);
    if (!id) return;
    this.router.navigate(['/messages'], { queryParams: { contactId: id } });
  }
