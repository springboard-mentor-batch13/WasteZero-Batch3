...
// NOTE: protect, requireAdmin, and adminLimiter are NOT applied here explicitly.
// Every request to /api/v1/admin/reports/* enters adminRoutes first (mounted at
// /api/v1/admin in server.js), which applies protect → requireAdmin → adminLimiter
// via router.use. Adding them again here caused adminLimiter to fire twice per
// request, consuming 2 of the budget's tokens instead of 1.
//
// reportRateLimiter IS kept on the download route (/:type) — it is a separate
// hourly limiter (10 downloads/hour) that is intentional and different from adminLimiter.
 ...
 *   500 — Export failure
 */
// Download route keeps its own reportRateLimiter (10 downloads/hour — separate
// from adminLimiter). adminLimiter still applies once via adminRoutes router.use.
router.get(
  '/:type',
  reportRateLimiter,
  reportTypeParam(),
